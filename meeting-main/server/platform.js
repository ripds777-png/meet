...
export function allowed(p,d,g,permission){return !!(p?.active&&p.roles.some(r=>['advisor','responsable'].includes(r))&&d&&p.societies.includes(d.society)&&g?.user_id===p.id&&g.dossier_id===d.id&&g.permissions.includes(permission));}
...
export async function saveData(p,d,data,eventName,extra={}){return rpc('meet_save_dossier',{p_id:d.id,p_expected:d.revision,p_data:data,p_actor:p.id,p_event:eventName,p_documents:documents(data),p_extra:extra});}
