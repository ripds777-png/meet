import {requestDraft,editRequest,approveRequest,addConstraint,constraintLevels} from '../workflow.js';
...
async function dossierMembers(id){const gs=await rows('meet_grants','dossier_id=eq.'+id);return gs.length?(await rows('meet_profiles','id=in.('+gs.map(g=>g.user_id).join(',')+')')).map(({id,name,email,active,roles,societies})=>({id,name,email,active,roles,societies})):[];}
...
async function storedParts(file){
 const parts=await allRows('meet_file_parts','file_id=eq.'+file.id+'&order=part.asc');const verified=[];
 for(let i=0;i<parts.length;i+=4){const batch=await Promise.all(parts.slice(i,i+4).map(async part=>{try{const r=await service('/storage/v1/object/authenticated/meet-private/'+part.object_key,{binary:true,headers:{Range:'bytes=0-0'}});const length=Number(r.headers.get('content-range')?.split('/')[1]||r.headers.get('content-length'));await r.body?.cancel();return length===part.bytes?part:null;}catch(e){if(e.status===404)return null;throw e;}}));verified.push(...batch.filter(Boolean));}return verified;
}
...
export default async function handler(req){try{
 ...
 if(action==='login'){
  ...
  await rpc('meet_touch_activity',{p_actor:p.id,p_login:true});
 ...
 if(action.startsWith('admin-')){
  hasRole(p,'admin');
  if(action==='admin-references')return json(200,{dossiers:(await allRows('meet_dossiers','select=id,name,client_id,society&order=name.asc,id.asc')).map(({id,name,client_id,society})=>({id,name,client_id,society})),grants:await allRows('meet_grants','order=dossier_id.asc,user_id.asc')});
  ...
  if(action==='admin-create'){
   const email=checkText(body.email,254).toLowerCase(),name=checkText(body.name);const password=temporaryPassword();
  ...
  if(action==='admin-reset'||action==='admin-revoke'){
   const q=(await rows('meet_profiles','id=eq.'+target))[0];if(!q)throw new Fault(404,'Compte absent.');const password=temporaryPassword();
  ...
  if(action==='admin-grant'){
   const dossierId=uuid(body.dossierId),permissions=body.permissions;if(!Array.isArray(permissions)||permissions.some(v=>!['read','write','call','assign','review','approve','transfer','supervise'].includes(v)))throw new Fault(400,'Permissions invalides.');
   await rpc('meet_set_grant',{p_actor:p.id,p_target:target,p_dossier:dossierId,p_permissions:permissions});return json(200,{ok:true});
  }
  throw new Fault(404,'Action inconnue.');
 }
 if(action==='list'){
  if(!p.roles.some(r=>['advisor','responsable','admin'].includes(r)))throw new Fault(403,'Un rôle opérationnel est requis pour consulter les dossiers.');
  const grants=await rows('meet_grants','user_id=eq.'+p.id);const ids=grants.filter(g=>g.permissions.includes('read')&&(p.roles.some(r=>['advisor','responsable'].includes(r))||g.permissions.includes('supervise'))).map(g=>g.dossier_id);if(!ids.length)return json(200,{dossiers:[],clients:[],notifications:[],calls:[],tasks:[],documents:[],grants:[],members:[]});
 ...
 const id=uuid(body.dossierId||url.searchParams.get('dossierId'));const d=await access(p,id,req.method==='GET'?'read':action==='start-call'||action==='segments'||action==='finish-call'||action==='call-config'?'call':'write');
 if(action==='access')return json(200,{ok:true});
 ...
 if(action==='detail')return json(200,{dossier:d,members:await dossierMembers(id),calls:await rows('meet_calls','dossier_id=eq.'+id+'&order=started_at.desc'),documents:await rows('meet_documents','dossier_id=eq.'+id+'&order=created_at.desc'),files:[...await allRows('meet_files','dossier_id=eq.'+id+'&order=created_at.desc,id.desc'),...linked.map(f=>({...f,linked:true}))],tasks:await rows('meet_tasks','dossier_id=eq.'+id+'&order=due.asc'),jobs:await rows('meet_jobs','dossier_id=eq.'+id+'&order=created_at.desc&limit=100'),audit:await rows('meet_audit','dossier_id=eq.'+id+'&order=created_at.desc&limit=200'),grants:await rows('meet_grants','dossier_id=eq.'+id)});
 ...
 if(action==='workflow'){
  if(body.revision!==d.revision)throw new Fault(409,'Le dossier a changé. Rechargez avant de modifier.');
  const data=structuredClone(d.data);data.requests ||= [];
  if(body.command==='request-create'){data.requests.push(requestDraft(data));}
  else if(body.command==='request-edit'){
   const index=data.requests.findIndex(x=>x.id===body.requestId);if(index<0)throw new Fault(404,'Demande absente.');
   const recipients=String(body.recipients||'').split(/[;,]/).map(x=>x.trim()).filter(Boolean);if(recipients.length>10||recipients.some(x=>! /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(x)))throw new Fault(400,'Destinataires invalides.');
   const documentIds=Array.isArray(body.documents)?body.documents:[],fileIds=Array.isArray(body.files)?body.files:[],versions=[];
   for(const docId of documentIds){const doc=(await rows('meet_documents','id=eq.'+uuid(docId)+'&dossier_id=eq.'+id))[0];if(!doc)throw new Fault(403,'Version hors dossier.');versions.push({id:doc.id,model:doc.model,version:doc.version,sourceRevision:doc.source_revision});}
   for(const fileId of fileIds){const file=(await rows('meet_files','id=eq.'+uuid(fileId)+'&dossier_id=eq.'+id))[0];if(!file||file.receipt!=='received')throw new Fault(403,'Pièce absente du dossier.');}
   data.requests[index]=editRequest(data.requests[index],{recipients,recipientVerified:body.recipientVerified===true,message:checkText(body.message,10000),documents:versions,files:fileIds},p.id);
  }else if(body.command==='request-approve'){
   hasRole(p,'responsable');await access(p,id,'approve');const index=data.requests.findIndex(x=>x.id===body.requestId);if(index<0)throw new Fault(404,'Demande absente.');
   const request=data.requests[index];for(const version of request.documents){const latest=(await rows('meet_documents','dossier_id=eq.'+id+'&model=eq.'+encodeURIComponent(version.model)+'&order=version.desc&limit=1'))[0];if(latest?.id!==version.id)throw new Fault(409,'Une nouvelle version documentaire existe. Recomposez le pack.');}
   try{data.requests[index]=approveRequest(request,p.id);}catch(e){throw new Fault(400,e.message);}
  }else if(body.command==='request-stop'){
   const request=data.requests.find(x=>x.id===body.requestId);if(!request)throw new Fault(404,'Demande absente.');request.reminders.stopped=true;request.status='stopped';request.approval=null;request.history.push({actor:p.id,at:new Date().toISOString(),event:'stopped',reason:checkText(body.reason,1000)});
  }else if(body.command==='constraint'){
   try{const c=addConstraint(data,body.constraint,p.id);if(c.ownerId){const owner=(await dossierMembers(id)).find(m=>m.id===c.ownerId&&m.active);if(!owner)throw new Fault(403,'Responsable non affecté.');}data.constraints.push(c);}catch(e){throw new Fault(e.status||400,e.message);}
  }else if(body.command==='constraint-rules'){
   hasRole(p,'responsable');await access(p,id,'review');const rules={};for(const key of Object.keys(constraintLevels))rules[key]=checkText(body.rules?.[key],500);data.constraintRules=rules;
  }else throw new Fault(400,'Commande de suivi inconnue.');
  return json(200,await saveData(p,d,data,'workflow.'+body.command));
 }
 if(action==='import'){
 ...
 if(action==='call-config'){
  hasRole(p,'advisor');const call=(await rows('meet_calls','id=eq.'+uuid(body.callId)+'&dossier_id=eq.'+id))[0];if(!call||call.advisor_id!==p.id||call.ended_at)throw new Fault(403,'Appel non modifiable.');
  const cfg=body.config;if(!cfg||![20,30,45,60,90,120].includes(cfg.duree))throw new Fault(400,'Durée invalide.');
  const next={...call.config,duree:cfg.duree,interlocutor:String(cfg.interlocutor||'').slice(0,1000),businessRole:String(cfg.businessRole||'Advisor').slice(0,100),agenda:Array.isArray(cfg.agenda)?cfg.agenda.slice(0,6):[]};
  await patch('meet_calls','id=eq.'+call.id,{config:next});await audit(p,'call.config.changed',{callId:call.id,previous:call.config,current:next},id);return json(200,{ok:true});
 }
 ...
 if(action==='file-init'){
  const fileId=uuid(body.fileId),old=(await rows('meet_files','id=eq.'+fileId))[0];if(old){if(old.dossier_id!==id||old.created_by!==p.id)throw new Fault(403,'Fichier non autorisé.');if(Number(old.bytes)!==Number(body.bytes)||old.parts!==Number(body.parts)||old.kind!==body.kind||old.call_id!==(body.callId||null))throw new Fault(409,'Le contenu ou le rattachement de cet envoi a changé.');return json(200,{...old,uploadedParts:(await storedParts(old)).map(x=>x.part)});}
 ...
 if(action==='upload-part'){
  const fileId=uuid(url.searchParams.get('fileId')),part=Number(url.searchParams.get('part')),file=(await rows('meet_files','id=eq.'+fileId+'&dossier_id=eq.'+id))[0];if(!file||file.created_by!==p.id||!['uploading','received'].includes(file.receipt)||!Number.isInteger(part)||part<0||part>=file.parts)throw new Fault(403,'Fragment non autorisé.');
  ...
  const old=(await rows('meet_file_parts','file_id=eq.'+fileId+'&part=eq.'+part))[0];if(old){if(old.sha256!==sha256)throw new Fault(409,'Fragment différent déjà sauvegardé.');const present=await storedParts({...file,parts:1});if(present.some(x=>x.part===part))return json(200,{ok:true});}
  ...
  if(!old)await insert('meet_file_parts',{file_id:fileId,part,sha256,bytes:bytes.byteLength,object_key});return json(200,{ok:true});
 }
 if(action==='file-complete'){
  ...
  const parts=await storedParts(file);if(parts.some((p,i)=>p.part!==i)||parts.length!==file.parts||parts.reduce((a,b)=>a+b.bytes,0)!==file.bytes)throw new Fault(409,'Des fragments sont encore absents.');
